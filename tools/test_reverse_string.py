...
def test_reverse_string_single_word():
    ...
    assert result == 'olleh', "The single word string should be reversed"

def test_reverse_string_special_characters():
    result = reverse_string('hello$%^&world')
    assert result == 'dlrow&^%$olleh', "The string with special characters should be reversed"

def test_reverse_string_empty_string():
    result = reverse_string('')
    assert result == '', "The empty string should return an empty string"
...
def test_reverse_string_list_input():
    with pytest.raises(TypeError, match="Input must be a string"):
